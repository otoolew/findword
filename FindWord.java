	...
	public FindWord()
	{
        ...
        while (true)
        {
           try
           {
               ...
               //fString="s1.txt";// for ease of debug
		...
		for (int i = 0; i < rows; i++)
		{
			...
			{
                            char let =Character.toLowerCase(rowString.charAt(j));
				theBoard[i][j] = let;
		...
		System.out.println("Please enter the phrase to search for:");              
                String []phrase = (inScan.nextLine().toLowerCase()).split(" ");
            for (int i=0;i<phrase.length;i++) {
                word = phrase[i];
                
                doWork(word, rows, cols, theBoard);
                if(result){
                    //System.out.println("Found a word!");
                    findWordNext(rows, col, word, loc, theBoard);
                }
            }
	}
        
        private boolean doWork(String word, int rows, int cols,char[][] theBoard){
                boolean found = false;            
                result=false;          
		while (!(word.equals(""))&&!found)
		{
			int x = 0, y = 0;
...
private boolean findWordNext(int r, int c, String word, int loc, char [][] bo)
	{
	...
	public boolean findWord(int r, int c, String word, int loc, char [][] bo)
	{
		...
		else  	// current character matches
		{
			bo[r][c] = Character.toUpperCase(bo[r][c]);  
                        //end=bo;
                                // Change it to upper case.  This serves two purposes:
				// 1) It will no longer match a lower case char, so it will
				//    prevent the same letter from being used twice
				// 2) It will show the word on the board when displayed
